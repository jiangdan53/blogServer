...
const {loginDb,updataAdminDb,whoamiDb} = require('../dao/adminDb')
...
exports.whomi = async function(userId){
...
exports.updataAdmin = async (newUserObnj)=>{
    newUserObnj.oblloginPwd = md(newUserObnj.oblloginPwd);
    newUserObnj.loginPwd = md(newUserObnj.loginPwd)
    // 查询用户是否存在 判断旧密码是否正确
const userObj = await loginDb({loginId:newUserObnj.loginId,loginPwd:newUserObnj.oblloginPwd})
if(userObj && userObj.dataValues){
    const result =  await updataAdminDb({id:userObj.id,oblloginPwd:newUserObnj.oblloginPwd,loginId:newUserObnj.loginId,loginPwd:newUserObnj.loginPwd,userName:newUserObnj.userName})
