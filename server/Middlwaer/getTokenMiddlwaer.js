 ...
 */ 
let getArr = [
    {methde:'GET',path:'/login/whoami'} ,
    {methde:'PUT',path:'/login/updataAdmin'},
...
module.exports = function(res,req,next){
    ...
    const token = verify(res) //将我们的请求头传递进入 以便拿到我们的token值 进行token校验
    if(!token) { //token校验失败或者没有token直接拒绝
       throw new  ValidationErr('token 未传递')
    }else{
        res.userId = token.id ;
        next()
    }
